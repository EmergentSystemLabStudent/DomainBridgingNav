...
class Trainer:
    ...
    def run(self, autocast: bool=True) -> None:
        ...
        for epoch in tqdm(range(self._epochs)):
            ...
            for minibatch in tqdm(self._dataloader):
                view_batch1, view_batch2, labels = minibatch[0].to(self._device), minibatch[1].to(self._device), minibatch[2].to(self._device)
                self._optimizer.zero_grad()
                
                if autocast:
                    with torch.cuda.amp.autocast():
                        p1, p2, z1, z2, logits1, logits2 = self._model(view_batch1, view_batch2)
                        
                        cosine_loss = -(self._cosine_sim(p1, z2).mean() + self._cosine_sim(p2, z1).mean())*0.5
                        classfier_loss = (self._cross_entropy(logits1, labels) + self._cross_entropy(logits2, labels))*0.5
                        loss = cosine_loss + classfier_loss
            ...
            validation_loss, validation_cosine_sim, validation_cross_entropy = self._validation_loss(autocast)
    ...
    def _validation_loss(self, autocast: bool=True) -> Tuple[float, float, float]:
        ...
        return sum(loss_log)/len(loss_log), sum(cosine_sim_log)/len(cosine_sim_log), sum(cross_entropy_log)/len(cross_entropy_log)
